...
/**
 * Generated class for the SubsubcategoryPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-subsubcategory',
  templateUrl: 'subsubcategory.html',
})
export class SubsubcategoryPage {
catData : any;
imgPath : any;
appSetng : any;
sldBranch : any;
...
public allMyFav = [];
  constructor(public navCtrl: NavController,
    ...
    this.serviceApi.getSubSubcategory(this.catId).subscribe(data => {
      this.catData = data;
  ...
  goNextPage(val){
  	if (val.subcategory=='YES') {
  		...
  		this.navCtrl.push(ProductsPage,{catgegoryName:val.categoryName, subSubCatId:val.categoryId, subCatId:'', catId: ''});
  	}
  }


  addToCart(prd){
    document.getElementById('addBtnIdsbb_'+prd.productId).style.display = "none";
    document.getElementById('qtyBtnIdsbb_'+prd.productId).style.display = "block";
    this.cart.addcart(prd,'','');
    this.crtTotal = this.cart.getcarttotal();
    this.crtItmCnt = this.cart.getItemCount();
    this.thsQty = this.cart.getSingleQty(prd.productId);
    document.getElementById('prdQtysbb_'+prd.productId).innerHTML = this.thsQty;
    this.isCartVal = true;
  }
  goCartPage(){
  ...
  plusQty(itm){
    ...
    document.getElementById('prdQtysbb_'+itm.productId).innerHTML = this.thsQty;
  }
  ...
  minusQty(itm){
    ...
    if (this.thsQty > 0) {
      ...
      document.getElementById('addBtnIdsbb_'+itm.productId).style.display = "block";
      document.getElementById('qtyBtnIdsbb_'+itm.productId).style.display = "none";
    }
